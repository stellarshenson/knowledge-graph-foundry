...
- **Warm-start band** - what: is the cached calibration still true - how (registered, band check not yet wired into ingest): first real window vs cached mean +-3 sigma with sigma MEASURED from repeat windows (never a fallback tolerance) - why: trust-but-verify; a changed setup recalibrates instead of silently underperforming
