...
The intent is especially critical during the first documents when the ontology is being discovered from scratch. Without intent, the LLM invents ad-hoc types that fragment the graph. With intent, even the free prompt (used for doc 1 before any types are discovered) produces domain-appropriate types. Benchmark v18 demonstrated this: doc 1 discovered all 12 final types with 0 new types needed from doc 2 onwards, because the intent steered extraction toward the right categories from the start.
...
**Empirical results** (CPAP benchmark, 10 documents, fluid mode):

The v09 run (first fluid mode) cured naturally at document 4 of 10. Stability metrics showed clear convergence: JSD below 0.02 from doc 2, Chao1 coverage from 0.881 to 0.956 at doc 4, Heaps' beta from 0.037 to 0.009, entropy delta from 0.0568 to 0.0002. The fluid phase discovered 31 entity types and 74 relationship types - all 8 seed types independently discovered, but with type proliferation (Standard/Regulatory_Standard/SafetyStandard as 3 types for 1 concept). Scored 81 hybrid (vs 88 seeded).

Subsequent iterations (v10-v17) addressed type proliferation, merge validation, and Bayesian resolution but introduced regressions - the score declined from 81 to 72 as parameter tuning fought the structural problem of no domain compass during early ontology discovery.

The v18 run (intent-driven discovery model) addressed all five structural failures. With intent prompt active and emerge threshold lowered to 1, doc 1 discovered all 12 final types and doc 2 added 0 new types. Curing triggered at doc 4 via heuristic (coverage=1.000, chao1=1.000, 0 new types in stability window). The cured ontology had 12 entity types and 14 relationship types. Final graph: 1035 entities, 3822 relationships, 158 chunks, 10 documents, 0 orphans. Scored 82 hybrid (deterministic 60/63, generative 3.4/5.0). Key improvements over v17: document coverage 75%->100% (proper doc/chunk creation with early curing), HAS_SPECIFICATION 0->passing (relationship types in constrained prompt from doc 1), singleton types 12-><3 (no pre-clustering pruning), type distribution 80%->100%. Remaining gap: cross-type duplicates at 56 (entities like Humidifier appearing as both Component and Accessory) and SUPPORTS_MODE=0.

The v19 run (Bayesian cross-type dedup) replaced the binary description-similarity gate with a multi-signal Bayesian posterior for cross-type resolution. The posterior combines name identity prior (0.8 for identical normalized names), description Jaccard likelihood ratio (floored at 0.3), embedding cosine likelihood ratio, and source chunk co-occurrence. Merge threshold: 0.6 (configurable via `cross_type_merge_threshold`). Also fixed the SUPPORTS_MODE benchmark query - the ontology has no Mode entity type, modes are Feature/Setting entities, so the query was matching nothing. Fixed `resolve_against_graph()` to use case-insensitive `toLower()` Neo4j query. Final graph: 1022 entities, 3779 relationships, 158 chunks, 10 documents. Scored 84 hybrid (deterministic 61/63, generative 3.6/5.0). Cross-type duplicates dropped from 56 to 39 (34 Bayesian merges approved, 99 blocked). The 39 remaining duplicates split into genuinely ambiguous entities (~20, where the type boundary is real) and entities lacking sufficient evidence signals (~19). Remaining gap: cross-type duplicates=39 (threshold <20), SleepStyle modes not linked, generative query_answerability stuck at 2/5 due to truncated judge context.

The v20 run (expanded judge context) made no pipeline changes - purely benchmark measurement fixes. Replaced the single-query query_answerability judge with a three-query approach: product/org outgoing relationships with 200-char descriptions (was 60-char), dedicated specification detail query returning `value`/`unit` properties, and mode/standard coverage query. Enriched cross_doc_resolution prompt with entity descriptions. Final graph: 1007 entities, 3496 relationships, 158 chunks, 10 documents. Scored 88 hybrid (deterministic 61/63, generative 4.0/5.0). query_answerability jumped 2/5 -> 4/5, confirming the 84 ceiling was a benchmark measurement problem. Remaining gap: cross-type duplicates at 52 (non-deterministic extraction variance), cross_doc_resolution generative stuck at 3/5 due to genuine Component/Accessory type ambiguity.

**Type proliferation fix (v10)** - root cause analysis identified three problems: (1) surface variants like Standard/Regulatory_Standard/SafetyStandard creating synonymous types, (2) ID normalization running before type enforcement producing stale IDs when types are remapped, and (3) cross-type resolution using a static hardcoded priority instead of data-driven frequency ranking. The fix implements two-layer type normalization. The deterministic layer adds `normalize_type_name()` - a PascalCase normalizer that splits on spaces, underscores, hyphens, and camelCase boundaries, applied everywhere type names are handled. The ontology buffer tracks a `_canonical_map` mapping normalized forms to first-seen raw forms, collapsing surface variants during accumulation (so "Safety Standard", "safety_standard", "SafetyStandard" all merge to one entry). The ID normalization ordering bug is fixed by moving `normalize_entity_ids()` after `_enforce_ontology_types()` in the extraction pipeline. Cross-type resolution now builds type priority dynamically from buffer frequency counts instead of using a static 8-type table. The LLM-assisted layer (`type_clustering.py`) makes one structured-output call at curing time with all discovered types, their frequencies, and the intent prompt - the LLM clusters semantic synonyms that deterministic normalization cannot catch (Standard vs RegulatoryStandard - different words, same concept). Type pruning before clustering was removed in v18 after analysis showed it deleted semantically important low-frequency types that the clustering LLM needed. With 1400 entities, a 0.5% threshold required 7+ entities per type - Mode (5 entities, present in all 10 documents) was incorrectly pruned. The LLM clustering step already handles type consolidation and can decide what to merge based on semantic understanding rather than frequency alone.
