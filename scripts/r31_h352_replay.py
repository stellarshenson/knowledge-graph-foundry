...
Checkpoints per chunk to results/r31/h352-{arm}.jsonl (detached-compute rule);
on resume completed chunks are skipped and the arm's wall-clock is flagged
tainted (call latencies remain valid).

Usage: python scripts/r31_h352_replay.py
"""
...
def main():
    ...
    out = Path(f"reports/r31-h352-{ts}.json")
    out.write_text(
        json.dumps(
            {
                "hypothesis": "R31-H352",
