...
scripts/experiments/r30_gpu_ramp.py (H363) - only the window logic is swapped:
...
Outputs: reports/experiments/r30/h388-fast-steps.jsonl (checkpoint),
reports/experiments/adjudicated/r30-h388-fast-ramp-<ts>.json. Parity is judged against the closed
H363 STEPs in reports/experiments/r30/h363-steps.jsonl; H347's knee detectors run
offline over the merged curve.

Usage: python scripts/experiments/r30_fast_ramp.py [step ...]   (default ladder 32 48 64 96 128)
"""
...
CHECKPOINT = Path("reports/experiments/r30/h388-fast-steps.jsonl")
...
def _ladder(steps, chunks, chunk_cycle, all_steps, engine, ex, purpose, ontology, active):
    ...
    out = Path(f"reports/experiments/adjudicated/r30-h388-fast-ramp-{ts}.json")
    out.write_text(
        json.dumps(
            {
                "hypothesis": "R30-H388",
