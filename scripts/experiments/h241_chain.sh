...
$PY scripts/experiments/h241_measure.py v1 2>&1 | tee -a "$LOG"

# 3. run the v2 arm (wipes neo4j4, init, ingest)
bash scripts/experiments/h241_arm.sh v2 2>&1 | tee -a "$LOG"

# 4. measure v2 against its LIVE graph
$PY scripts/experiments/h241_measure.py v2 2>&1 | tee -a "$LOG"
