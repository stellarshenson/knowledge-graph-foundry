...
**Core** (required): `Document`, `Chunk`, `Entity`, `FactNode`, `OntologyType`, `Source`, `SchemaVersion`
...
**Document score**: 20.4 (lower = better, max 136)

| # | Concern | Risk | Score | Residual | How addressed |
|---|---------|------|-------|----------|---------------|
| 1 | Agent overuse | 20 | 90% | 2.0 | Section 2: interactive vs autonomous mode, `--batch` flag, checkpoint annotations, batch decision logging |
| 2 | Optional features as core | 15 | 80% | 3.0 | Section 8 core vs extension node types, clear required vs optional distinction |
...
| 4 | Schema inference instability | 12 | 90% | 1.2 | Section 7.3 rewritten: schema-as-configuration principle (conversation is UX, saved file is authority), lockfile at `.kg-builder/schema.lock`, deterministic field profile baseline, schema versioning with `SchemaVersion` nodes in Neo4J, `CREATED_UNDER` linking, schema recovery via `kg init` |
...
| 8 | OWL reasoning complexity | 6 | 70% | 1.8 | Optional, off by default, Cypher alternative provided |
| 9 | No scale targets | 12 | 70% | 3.6 | Scale targets in Section 2, architecture change notes for larger scale |
...
**Top gaps**: #9 scale benchmarking (3.6), #2 phased roadmap (3.0), #1 batch report format example (2.0), #5 confidence (2.0), #6 Levenshtein details (1.8)
