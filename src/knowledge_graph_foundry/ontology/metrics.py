...
Ported from v1 (empirically validated on the CPAP corpus): Shannon entropy
delta, KL/JS divergence, Heaps beta, Zipf R2, Gini, Chao1 and ACE coverage
computed from type frequency distributions after every document. Stdlib-only
and decision-free - the curing detector and drift engine consume these.
"""
...
class StabilityMetrics:
    ...
    def record(self, frequencies: dict[str, int]) -> dict[str, float]:
        ...
        emit("curing.metrics", doc_index=len(self._history) - 1, **dict(metrics))
