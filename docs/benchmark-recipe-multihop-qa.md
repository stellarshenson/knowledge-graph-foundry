...
- **Detached compute** - every ingest and eval run launched via setsid/nohup, teed to `logs/`, checkpointed to `results/bench/`; runs must survive the driver
...
- **QA adapter** - benchmark question -> KGF query; captures retrieved passages + final answer per question to `results/bench/<dataset>-answers.jsonl`
...
- [ ] QA adapter built; answers checkpoint to `results/bench/`
