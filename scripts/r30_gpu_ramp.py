...
Outputs: results/r30/h363-steps.jsonl (checkpoint), reports/r30-h363-ramp-<ts>.json.
H347's knee detectors run offline over this output.

Usage: python scripts/r30_gpu_ramp.py [step ...]   (default ladder 16..96)
"""
...
CHECKPOINT = Path("results/r30/h363-steps.jsonl")
...
def main():
    ...
    out = Path(f"reports/r30-h363-ramp-{ts}.json")
    out.write_text(
        json.dumps(
            {
                "hypothesis": "R30-H363",
