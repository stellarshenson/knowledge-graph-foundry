"""Small-rung smoke verification (task #83): counts, question channel, probe, timing."""
...
drv = GraphDatabase.driver("bolt://172.19.0.9:7687", auth=("neo4j", "kgfoundry"))
...
events = []
with open(PROJ / "logs/bench-small-events.jsonl") as f:
    for line in f:
        try:
...
settings = load_settings(PROJ / "config/experiments/config-bench-small.yml")
...
v["effective_uri_is_throwaway"] = (
    "PASS" if "172.19.0.9" in settings.neo4j.uri else "FAIL"
)
