...
Aggregates the four per-arm/run stats files written by scripts/experiments/r29_measure.py and
applies the PRE-REGISTERED R29-H340 decider VERBATIM (docs/experiments/
kgf-redesign-experiments.md line 3682; mirrored in scripts/experiments/r29_chain.sh header).
...
Pure analysis over reports/experiments/adjudicated/r29-{v1,v2}-run{1,2}-stats.json - no GPU, no engine, no graph
write. Safe to run mid-chain: prints a PENDING preview until all four checkpoints exist.

Usage: python scripts/experiments/r29_decide.py
"""
...
def load():
    ...
    for a in ARMS:
        for r in RUNS:
            p = Path(f"reports/experiments/adjudicated/r29-{a}-run{r}-stats.json")
            if p.exists():
