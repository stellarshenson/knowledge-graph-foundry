...
- **Resolution guide evolution**: when `resolution_guide_evolution` is enabled, the buffer self-evolves during ingestion - `record_cross_type_stats()` triggers `evolve_type_hierarchy()` and `evolve_resolution_guide()` whenever encounter frequency reaches the threshold. A final `_maybe_evolve()` runs before flush to catch patterns from the last document. The `resolution_intent` (user-authored prior) is preserved verbatim and never modified. This creates a knowledge accumulation loop where each run teaches the ontology about its own ambiguities
