...
Writes: results/bench/progressive-probe-trajectory.jsonl (append, one record
...
TRAJECTORY = Path("results/bench/progressive-probe-trajectory.jsonl")
...
# every ladder slice file: doc name '<file>#rowN' maps to the title via its own file
_SLICES = {
...
def ingested_titles(f) -> set[str]:
    """KGFDocument.name is '<file>#rowN' - map back to the corpus title
    through whichever ladder slice file the doc came from."""
    ...
    with Foundry(st) as f:
        while True:
            ...
            print(
                ...
                f"{len(eligible)} eligible, {n_pass} pass / {n_fail} fail, "
