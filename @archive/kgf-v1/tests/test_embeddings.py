...
def _entity(name: str = "Test", entity_type: str = "Product") -> Entity:
    return Entity(id=f"e_{name}", name=name, type=entity_type, description="A test entity")
...
class TestBedrockProvider:
    ...
    def test_populates_embedding_field(self, mock_boto3):
        ...
        mock_body = MagicMock()
        mock_body.read.return_value = json.dumps({"embedding": fake}).encode()
        mock_client.invoke_model.return_value = {"body": mock_body}
        mock_boto3.Session.return_value.client.return_value = mock_client

        result = generate_embeddings([_entity("CPAP"), _entity("Humidifier")], provider="bedrock")
    ...
    def test_input_text_format(self, mock_boto3):
        ...
        generate_embeddings([entity], provider="bedrock", fallback=None)
    ...
    def test_per_entity_failure_after_first_success_continues(self, mock_boto3):
        ...
        result = generate_embeddings(
            [_entity("A"), _entity("B")], provider="bedrock", fallback=None
        )
        assert result[0].embedding == fake
...
class TestLocalProvider:
    def test_local_provider_uses_sentence_transformers(self):
        ...
        with patch.dict(sys.modules, {"sentence_transformers": fake_module}):
            result = generate_embeddings(
                [_entity("A"), _entity("B")], provider="sentence-transformers"
            )
    ...
    def test_local_provider_input_text_format(self):
        ...
        with patch.dict(sys.modules, {"sentence_transformers": fake_module}):
            generate_embeddings([entity], provider="sentence-transformers")
    ...
    def test_local_model_cached_across_calls(self):
        fake_model = MagicMock()
        fake_model.encode.return_value = [[0.1] * 384]
        ...
        with patch.dict(sys.modules, {"sentence_transformers": fake_module}):
            generate_embeddings([_entity("A")], provider="sentence-transformers")
            generate_embeddings([_entity("B")], provider="sentence-transformers")

        # Model constructed only once, reused across runs
...
class TestProviderFallback:
    ...
    def test_no_fallback_when_fallback_is_none(self, mock_boto3):
        ...
        with pytest.raises(Exception, match="AWS auth failed"):
            generate_embeddings([_entity("A")], provider="bedrock", fallback=None)

    @patch("knowledge_graph_foundry.extraction.embeddings.boto3")
    def test_fallback_partial_embeddings_cleared_before_retry(self, mock_boto3):
        """When primary fails mid-way, any partial embeddings must be cleared before fallback."""
        fake = [0.1] * 1024
        mock_client = MagicMock()
        good_body = MagicMock()
        good_body.read.return_value = json.dumps({"embedding": fake}).encode()
        # First call fails immediately = provider-level failure
        ...
        with patch.dict(sys.modules, {"sentence_transformers": fake_module}):
            result = generate_embeddings(
                [entity], provider="bedrock", fallback="sentence-transformers"
            )
...
class TestRunProviderLock:
    ...
    def test_locked_provider_does_not_fall_back(self, mock_boto3):
        ...
        generate_embeddings([_entity("A")], provider="bedrock")
        ...
        with pytest.raises(Exception, match="network down"):
            generate_embeddings(
                [_entity("B")], provider="bedrock", fallback="sentence-transformers"
            )

    def test_empty_list_does_not_lock_provider(self):
        """Empty input short-circuits without locking a provider."""
        result = generate_embeddings([], provider="sentence-transformers")
        assert result == []
        assert emb_mod._active_provider is None


class TestProviderResolution:
    def test_model_name_amazon_infers_bedrock(self):
        """Model starting with 'amazon.' infers bedrock provider."""
        provider, _ = emb_mod._resolve_provider(None, "amazon.titan-embed-text-v2:0")
        assert provider == "bedrock"

    def test_model_name_all_infers_local(self):
        """Model starting with 'all-' infers sentence-transformers provider."""
        provider, _ = emb_mod._resolve_provider(None, "all-MiniLM-L6-v2")
        assert provider == "sentence-transformers"

    def test_explicit_provider_overrides_model_inference(self):
        provider, _ = emb_mod._resolve_provider(
            "sentence-transformers", "amazon.titan-embed-text-v2:0"
        )
        assert provider == "sentence-transformers"

    def test_default_provider_when_no_hints(self):
        """No provider, no model -> defaults to bedrock (current behavior)."""
        provider, model = emb_mod._resolve_provider(None, None)
        assert provider == "bedrock"
        assert model == "amazon.titan-embed-text-v2:0"

    def test_default_model_for_sentence_transformers(self):
        provider, model = emb_mod._resolve_provider("sentence-transformers", None)
        assert provider == "sentence-transformers"
