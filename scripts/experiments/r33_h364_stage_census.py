...
Inputs: reports/experiments/adjudicated/r32-h354-census-*.json (target golds), tests/probes/cpap-probe-set.yml,
the h212-v2 graph (bolt://172.19.0.100:7687). Output: reports/experiments/adjudicated/r33-h364-stage-census-<ts>.json
...
CENSUS = Path(sys.argv[1]) if len(sys.argv) > 1 else Path("reports/experiments/adjudicated/r32-h354-census-20260710T125440Z.json")
...
def main():
    ...
    with Foundry(st) as f:
        ...
        out = Path(f"reports/experiments/adjudicated/r33-h364-stage-census-{ts}.json")
