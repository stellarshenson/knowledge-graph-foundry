"""R22-H240(b) per-arm measurement: recall@16 (persist JSON) + graph stats.
Run from repo root AFTER an arm's ingest, BEFORE the next wipe.
Usage: python scripts/experiments/h240b_measure.py <enum|mention>
"""
import hashlib
import json
import sys
...
RECALL_JSON = f"reports/experiments/adjudicated/h240b-{ARM}-recall.json"
STATS_JSON = f"reports/experiments/adjudicated/h240b-{ARM}-stats.json"
...
rec = recall_at_k(URI, 16, f"{ARM}-h240b")
