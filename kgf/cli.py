...
def main_callback(ctx: typer.Context):
    ...
    if ctx.invoked_subcommand is None:
        from kgf.tui.app import KGBuilderApp
...
def ingest(
    ...
    from kgf.events import signals
    from kgf.events import types as etypes
    from kgf.extraction.extract import LLMAuthError
    from kgf.extraction.unstructured import ExtractionFailedError
    ...
    try:
        ...
        if detection["has_metanode"]:
            ...
            if stale_fsm_state == GraphState.CURING and stale_consolidation is None:
                ...
                from kgf.fsm import delete_fluid_results, delete_fluid_state
            ...
            if force_recure or (stale_run_id is not None and stale_consolidation is not None):
                from kgf.fsm import update_control_metanode as _update_meta
...
def _update_metanode_safe(config, ctx) -> None:
    ...
    try:
        from kgf.fsm import create_control_metanode
...
def _write_fluid_result_safe(config, fsm_ctx, doc_name: str, doc_index: int, result) -> None:
    ...
    if not fsm_ctx:
        return
    try:
        from kgf.fsm import write_fluid_result
...
def _write_fluid_state_safe(config, fsm_ctx, detector, metrics_tracker, deferred_buffer) -> None:
    ...
    if not fsm_ctx:
        return
    try:
        from kgf.fsm import write_fluid_state
...
def _read_fluid_results_safe(config, fsm_ctx):
    ...
    if not fsm_ctx:
        return []
    try:
        from kgf.fsm import read_fluid_results
...
def _read_fluid_state_safe(config, fsm_ctx):
    ...
    if not fsm_ctx:
        return None
    try:
        from kgf.fsm import read_fluid_state
...
def _check_fluid_cache_exists_safe(config, graph_id: str) -> bool:
    ...
    try:
        from kgf.fsm import check_fluid_cache_exists
...
def _ingest_direct(
    ...
    *,
    ...
    from kgf.extraction.unstructured import ingest_document
    from kgf.loading.loader import load_extraction
...
def _ingest_fluid(
    ...
    *,
    ...
    from kgf.events import types as etypes
    from kgf.extraction.unstructured import ingest_document
    from kgf.loading.loader import (
        load_doc_chunks,
        load_extraction,
    ...
    from kgf.curing.adaptive import CalibrationHotLoader
    ...
    for i, file_path in enumerate(files):
        ...
        if should_cure:
            ...
            from kgf.curing.type_clustering import (
                apply_type_mapping,
                cluster_types,
            )
            from kgf.extraction.dedup import normalize_entity_ids
            ...
            from kgf.curing.adaptive import AdaptivePriorState
...
def _load_calibrator_safe(config, fsm_ctx, model_type: str):
    ...
    try:
        ...
        from kgf.fsm import read_calibration_curve
...
def _fit_and_persist_calibration(
    ...
    if type_metrics and gt_pairs:
        from kgf.curing.observation import compute_spearman_correlations
...
def _persist_calibration_curve_safe(config, fsm_ctx, calibrator, model_type: str) -> None:
    ...
    try:
        ...
        from kgf.fsm import write_calibration_curve
...
def init():
    ...
    from kgf.settings.defaults import DEFAULTS
